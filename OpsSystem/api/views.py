...
def get_project_current_version(request):
    ...
    for repo in repos:
        try:
            test_cur_version = DeployInfoModel.objects.filter(repoName=repo).filter(target='test').order_by(
                '-date').first()
            pre_cur_version = DeployInfoModel.objects.filter(repoName=repo).filter(target='pre').order_by(
                '-date').first()
            online_cur_version = DeployInfoModel.objects.filter(repoName=repo).filter(target='online').order_by(
                '-date').first()
            tmp_dict = {}
            if test_cur_version:
