...
def check_svn_validated(user, password, url):
    ...
    svn_cmd = ''' svn info --non-interactive  --username="%s" --password="%s" %s &>/tmp/.svn_32197''' % (
    user, password, url)
...
def deploy_git_code(repo_name,
    ...
    if not os.path.exists(checkout_code_parent_dir + '/' + code_dir + '/.git/refs/heads'):
        logfunc(log_file, 'Error',
                'the revision file do not exists! :%s' + checkout_code_parent_dir + '/' + code_dir + '/.git/refs/heads')
    ...
    cmd_get_diff_file = ''' cd %s/%s && git diff --name-status %s %s|sed "s/^\s\+//g" >%s ''' % (
    checkout_code_parent_dir,
    code_dir,
    revision,
    lastest_revision,
    diff_file)
...
def deploy_svn_code(repo_name,
    ...
    cmd_checkout_code = '''export LC_ALL=zh_CN.UTF-8 && cd %s && svn checkout  --force %s && cd %s && svn cleanup ''' % (
    checkout_code_parent_dir,
    svn_cmd_args,
    code_dir)
    ...
    if int(revision) < int(lastest_revision):
        diff_file = '/tmp/.' + str(random.randint(10000, 1000000)) + '.log'
        cmd_get_diff_file = ''' cd %s/%s && svn diff %s -r%s:%s --summarize|sed "s/^\s\+//g" >%s ''' % (
        checkout_code_parent_dir,
        code_dir,
        simple_args,
        revision,
        ...
        if cmd_res != 0:
            return False, u'获取更新文件失败！！！', log_file
        cmd_dealwith_diff_file = ''' cat %s |egrep -v '^$' |awk '/^[^D]/{print $2}' > %s.log ''' % (
        diff_file, diff_file)
        logfunc(log_file, 'INFO', 'get dealwith file cmd: ' + cmd_dealwith_diff_file)
        cmd_res = os.system(cmd_dealwith_diff_file)
        logfunc(log_file, 'INFO', 'cmd run return code: ' + str(cmd_res))
...
def upload_code_with_no_password(code_dir,
    ...
    for ii in ip:
        ...
        cmd_upload = ''' rsync -e 'ssh -p %d' -avlpP %s --files-from=%s %s %s:%s &>/tmp/.up.log ''' % (
        _SSH_PORT, exclude_args,
        diff_file,
        code_dir,
        ii,
        wwwDir)
...
def rollback(repo_name, target, cur_version, roll_version, user, password):
    ...
    try:
        ...
        if repo_type == 'git':
            code_path = checkout_dir + '/' + repo_name + '_' + target + '/' + \
                        repo_addr[repo_addr.rfind('/') + 1:].split('.')[0]
...
def rollback_git_code(repo_name, roll_version, path, rsync_args, rsync_target, password):
    ...
    get_diff_file_cmd = r''' cd %s && git pull origin master && git diff --name-status HEAD %s ''' % (
    path, roll_version)
...
def rollback_svn_code(repo_name, roll_version, cur_version, path, rsync_args, rsync_target, password, svn_user,
                      svn_password):
    global ROLL_LOG_FILE
    log = ROLL_LOG_FILE
    os.system('rm -f %s && touch %s ' % (log, log))
    ...
    get_diff_cmd = ''' cd %s && svn revert -R . ; svn diff %s --summarize -r%s:%s |egrep '^M|^A' |awk '{print $NF}' >/tmp/.s ''' % (
    path,
    svn_args,
    cur_version,
    roll_version)
...
def judge_rollback_version_exist(project, target, cur_version, roll_version):
    try:
        roll_ver = DeployInfoModel.objects.filter(repoName=project).filter(target=target).filter(
            revision=roll_version).first()
        cur_ver = DeployInfoModel.objects.filter(repoName=project).filter(target=target).filter(
            revision=cur_version).first()
        if roll_ver:
...
def upload_code_with_password(code_dir, password, rsync_target, diff_file, exclude_dir, log_file):
    ...
    exclude_args = exclude_dir
    cmd_upload = ''' rsync  -e 'ssh -p %d' -avlpP %s --files-from=%s %s %s &>/tmp/.up.log ''' % (
    _SSH_PORT, exclude_args,
    diff_file,
    code_dir,
    rsync_target)
...
if __name__ == '__main__':
    ...
    print
    deal_with_exclude('dd.log;          ;haha;  ;makelog;')
